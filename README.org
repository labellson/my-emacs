...
Activate =pyvenv-mode= in =python-mode=
...
write =Javascript= code. In Emacs 27 =j2-mode= supports React =.jsx= so is
activated as a minor mode when =web-mode= or =js-mode= are activated.
...
For a comparable IDE experience it is possible to use Microsoft Language Server
Protocol in Emacs. The main objective of this project is to allow programming
language support to be implemented independetly of any given editor. To make it
work it is necessary to download each language respective server.

#+BEGIN_SRC emacs-lisp :results silent
(use-package lsp-mode
  :ensure t
  :hook (((typescript-mode python-mode) . lsp)
         (lsp-mode . lsp-enable-which-key-integration))
  :commands lsp
  :config

  ;; Read lsp-mode performance section
...
This package adds a collection of Evil bindings that =Evil= does not cover by
default.
...
** =restclient-mode=
This mode lets you explore and test HTTP REST webservices from plain
text. Evaluate the requests with =C-c C-c= and display the results as a
pretty-printed XML or JSON.
...
Thanks to =ob-restclient= is possible to provide support on =org-babel= and add
snippets with requests in your =org-mode= files.
...
Auto-completion for HTTP methods and headers in =restclient-mode= is provided
by =company-restclient=
...
** Literate Calc Mode
Display inline calc results as overlays in markup modes as =org-mode=. It is
possible to activate a minor mode =literate-calc-minor-mode= and use the
overlays.
