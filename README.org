...
** COMMENT =Irony-mode=
Is an emacs minor mode that indexes C/C++ code improving the editing
experience and therefore offers some IDE-like functionality.

It works by using a combination of an Emacs package and a C++ program
=irony-server= exposing =libclang=. =Irony= installs and runs automatically the
=irony-server=. So, is easy to configure.

=Irony= does not know how to work with header files, because compilation
database provides compilation information for source files but not for
headers. But luckily, =irony= has fallback options. So, lets use
=.clang_complete= file as a fallback option for headers files.
...
To enable code completion with company let's install the backend.
...
For live code checking =flycheck-irony= must be installed.
...
For =eldoc= integration with irony. =eldoc= will show the argument list of the
function you are currently writing.
