...
*** =RTags=
Is a client/server application that indexes C/C++ code. What it does is it
actually understands your C++ project and therefore offers some IDE-like
functionality for emacs.

RTags consist of =rc= the client, =rdm= the daemon that does indexing and emacs
=rtags= package that interacts with =rc=. In order to install =rc= and =rdm= it
can be done automatically by running ~rtags-install~. Or, manually as [[https://github.com/Andersbakken/rtags][RTags]]
GitHub repository explains.
...
=RTags= can use =Ivy= to view some results to do so just need install
=ivy-tags= and set the display backend to =ivy= with ~rtags-display-result-backend~.
...
For live code checking =flycheck-rtags= must be installed.
